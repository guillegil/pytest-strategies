...
def sample_config() -> dict[str, int]:
    ...
    return {"max_retries": 3, "timeout": 30, "batch_size": 100}
...
@Strategy.strategy("fixture_compatible_strategy")
def test_strategy_with_cleanup_fixture(user_id: int, operation: str, resource_with_cleanup: dict):
    ...
    resource_with_cleanup["data"].append({"user_id": user_id, "operation": operation})
...
def test_strategy_with_request_fixture(
    user_id: int, operation: str, request: pytest.FixtureRequest
):
...
@Strategy.strategy("fixture_compatible_strategy")
def test_strategy_with_tmp_path(user_id: int, operation: str, tmp_path: pytest.TempPathFactory):
...
@Strategy.strategy("fixture_compatible_strategy")
def test_strategy_with_monkeypatch(user_id: int, operation: str, monkeypatch: pytest.MonkeyPatch):
...
@Strategy.strategy("fixture_compatible_strategy")
def test_strategy_with_capsys(user_id: int, operation: str, capsys: pytest.CaptureFixture):
